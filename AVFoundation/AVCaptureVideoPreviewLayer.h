...
@interface AVCaptureVideoPreviewLayer : CALayer //录制视图预览层
{
@private
...
*/
+ (id)layerWithSession:(AVCaptureSession *)session; //通过录制会话创建预览图层
...
/*!
 ...
 @result
    A newly initialized AVCaptureVideoPreviewLayer instance.
*/
- (id)initWithSession:(AVCaptureSession *)session; //通过录制会话创建预览图层
...
/*!
 ...
 @result
    A newly initialized AVCaptureVideoPreviewLayer instance.
*/
+ (id)layerWithSessionWithNoConnection:(AVCaptureSession *)session NS_AVAILABLE(10_7, 8_0); //创建一个没有连接的预览图层
...
/*!
 ...
 @result
    A newly initialized AVCaptureVideoPreviewLayer instance.
*/
- (id)initWithSessionWithNoConnection:(AVCaptureSession *)session NS_AVAILABLE(10_7, 8_0);//创建一个没有连接的预览图层
...
*/
@property (nonatomic, retain) AVCaptureSession *session;//当前会话
...
/*!
 ...
 @discussion
    The session is retained by the preview layer.
*/
- (void)setSessionWithNoConnection:(AVCaptureSession *)session NS_AVAILABLE(10_7, 8_0);//设置一个没有连接的会话
...
*/
@property (nonatomic, readonly) AVCaptureConnection *connection NS_AVAILABLE(10_7, 6_0);//已经连接的会话连接
...
*/
@property (copy) NSString *videoGravity; //录制视频的显示方式
...
*/
- (CGPoint)captureDevicePointOfInterestForPoint:(CGPoint)pointInLayer NS_AVAILABLE_IOS(6_0);//把预览图层上pointInLayer的坐标转变成captureDevicelayer的坐标
...
*/
- (CGPoint)pointForCaptureDevicePointOfInterest:(CGPoint)captureDevicePointOfInterest NS_AVAILABLE_IOS(6_0);//把captureDevicelayer的坐标 转变成预览图层上的坐标
 ...
 */
- (CGRect)metadataOutputRectOfInterestForRect:(CGRect)rectInLayerCoordinates NS_AVAILABLE_IOS(7_0);//把预览图层上的rect 转变到输出AVCaptureMetadataOutput上的坐标
 ...
 */
- (CGRect)rectForMetadataOutputRectOfInterest:(CGRect)rectInMetadataOutputCoordinates NS_AVAILABLE_IOS(7_0);//把AVCaptureMetadataOutput上的坐标转变到预览图层的坐标
...
*/
- (AVMetadataObject *)transformedMetadataObjectForMetadataObject:(AVMetadataObject *)metadataObject NS_AVAILABLE_IOS(6_0);//把AVMetadataObject
...
*/
@property (nonatomic, readonly, getter=isOrientationSupported) BOOL orientationSupported NS_DEPRECATED_IOS(4_0, 6_0);//支持取向
...
*/
@property (nonatomic) AVCaptureVideoOrientation orientation NS_DEPRECATED_IOS(4_0, 6_0);//预览图层 显示方向，可以设置，只有orientationSupported == YES，才能设置 orientation
...
*/
@property (nonatomic, readonly, getter=isMirroringSupported) BOOL mirroringSupported NS_DEPRECATED_IOS(4_0, 6_0);//是否支持镜像
...
*/
@property (nonatomic) BOOL automaticallyAdjustsMirroring NS_DEPRECATED_IOS(4_0, 6_0);//自动调整镜像
...
*/
@property (nonatomic, getter=isMirrored) BOOL mirrored NS_DEPRECATED_IOS(4_0, 6_0);//开启镜像，只有isMirroringSupported = YES,automaticallyAdjustsMirroring = YES
